...
A fail-over client for the always-free (>13 B) model providers in MODELS_CONFIG.
This router automatically tries different LLM providers when one fails or hits rate limits.
Drop this file into your project and use it as shown below.

Requirements:
```
pip install langchain-openai langchain-google-genai langchain-mistralai langchain-fireworks langchain-groq langchain-together
```
...
class MyStructuredModel(BaseModel):
    property_1: str = Field(description="Description of property 1")
    property_2: List[str] = Field(description="Description of property 2")
```

Call it either via:
```python
router.with_structured_outputs(MyStructuredModel).invoke("Your prompt here")
```
...
```python
router.invoke(
    "Tell me a short joke as JSON.",
    schema=MyStructuredModel
)
```
"""
import time
